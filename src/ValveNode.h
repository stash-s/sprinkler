...
const int NUMBER_OF_VALVES = 5;
const uint8_t valvePins[NUMBER_OF_VALVES] = {D1, D2, D5, D6, D7};

/**
 * Use cases
 *
 * 1. Switch on sprinkler and let it stay on for a time given in seconds.
 *    - all other sprinklers are switched off. Controller publishes new state of
 * each valve: on/off state and timeout
 ...
 *    - sprinkler B is closed and timeout is set to zero. Both values are
 * published
...
inline bool ValveNode::isValidRange(const HomieRange &range) {
    ...
    if (range.index < 1 || range.index > NUMBER_OF_VALVES) return false;

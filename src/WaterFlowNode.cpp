...
WaterFlowNode::WaterFlowNode(const char *id, const char *name, uint8_t pinNumber)
    : HomieNode(id, name, "water-flow", false),
      sensorPin(pinNumber)
{
}
...
void WaterFlowNode::setup()
{
    ...
    advertise("flow").setName("Incremental flow").setDatatype("float").setUnit("litres").setRetained(false);
    advertise("beats").setName("Incremental beats").setDatatype("integer").setUnit("beats").setRetained(false);
    advertise("cumulative-beats").setName("Cumulative beats").setDatatype("integer").setUnit("beats").setRetained(true);
}
